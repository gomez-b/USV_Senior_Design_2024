...
#define ARRAY_SIZE 13  // Define the size of the array//without V and I its 11
        ...
        case MAVLINK_MSG_ID_GPS_RAW_INT:
          {
            mavlink_gps_raw_int_t gps_raw_int;
            mavlink_msg_gps_raw_int_decode(&msg, &gps_raw_int);
            data_array[8] = gps_raw_int.lat;
            data_array[9] = gps_raw_int.lon;
...
void logDataToSD(String current_voltage, String current_current, String current_day, String current_month, String current_year,String current_hour,String current_minute, String current_second, String current_latitude, String current_longitude, String current_temp,String current_salinity, String current_depth ) {
  ...
  if (dataFile) {
    String labels[] = { "Voltage","Current","Date", "Time", "Latitude", "Longitude","Temperature", "Salinity", "Depth" };
    String values[] = { current_voltage,current_current,current_day + "/" + current_month + "/" + current_year, current_hour + ":" + current_minute + ":" + current_second, current_latitude, current_longitude,current_temp, current_salinity, current_depth};
...
void readTemperatureAndSalinity() {
  ...
  Temp_Voltage = Temp_Analog / 1023.0 * 5.0;        // ADC conversion for Temp
                                                    // Serial.println(Temp);
                                                    // delay(1000);
  data_array[10] = String(Temp);

  float Salinity_Analog = analogRead(Salinity_Analog_Pin);       // reads A0 (Salinity)
  Salinity_Voltage = Salinity_Analog / 1023.0 * 5.0;             // ADC conversion for Salinity
  Two_point_calibration = (12.7793 * Salinity_Voltage) - 0.236921;  // calibration equation from QuestLab

  data_array[11] = String(Two_point_calibration);
}
...
void readPingData() {
  ...
  if (ping.update()) {
    ...
    depth = ping.distance();  // Convert distance to millimeters and then to feet
